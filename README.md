...
(seleniumによる操作は一部のフォーマットのみ対応です。)check Availability→カレンダー選択→枚数選択→Register
...
### API制限エラー（429）が発生する場合

`POLL_INTERVAL`の値を大きくしてください（推奨: 2.0秒以上）

### ブラウザの自動操作が失敗する場合

- ChromeDriverのバージョンがChromeブラウザと一致しているか確認してください
- エラー時にスクリーンショットが保存されるので、確認してください

### 環境変数が読み込まれない場合

- `.env`ファイルがプロジェクトのルートディレクトリに存在するか確認してください
- `python-dotenv`がインストールされているか確認してください

## ライセンス

このプロジェクトは個人利用を目的としています。Eventbriteの利用規約を遵守してご利用ください。

## 免責事項

このツールは教育目的で作成されています。使用に際しては、Eventbriteの利用規約およびAPI利用規約を遵守してください。このツールの使用によって生じたいかなる問題についても、作者は責任を負いません。
